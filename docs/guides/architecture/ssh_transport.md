...
- `reopen_shell_pty` 采用同样的重试边界：channel-open 失败且判定为陈旧连接时驱逐并重连一次，之后不再重试。
...
所有现有 Tauri command 名称、参数、返回结构及事件 payload 保持不变。1.5.4 新增 `reopen_shell_pty`（入参 `{sessionId}`，返回原 `ShellSession`），用于在已有 session id 上重建 PTY channel。
...
- 断连后点重连：确认标签页数量不变（不产生孤儿标签页），工作目录与 SFTP 路径保留。
- 终端内 `exit` 退出 shell：标签页保留并显示断连遮罩，重连可开新 shell。
- 终端 Ctrl+Shift+C / Ctrl+Shift+V 复制粘贴；无选区时 Ctrl+Shift+C 应发出 `^C`。
