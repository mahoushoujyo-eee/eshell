...
use super::{pty, service, transport};
use crate::models::*;
...
impl Fixture {
    async fn new() -> Self {
        ...
        let key = russh::keys::PrivateKey::from_openssh(transport::test_support::TEST_PRIVATE_KEY)
            .unwrap();
        let fingerprint = key
            .public_key()
...
async fn private_key_auth_and_bad_key_password_fallback() {
    timeout(Duration::from_secs(10), async {
        ...
        tokio::fs::write(&key_path, transport::test_support::TEST_PRIVATE_KEY)
            .await
            .unwrap();
