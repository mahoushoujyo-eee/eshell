//! ACP session runner: spawns the external agent subprocess, drives the
//! JSON-RPC session and translates ACP events onto the `acp-agent-stream`
//! Tauri event.
...
use crate::domain::agent::model::{
    AcpAgentCapsView, AcpAgentInfoView, AcpAuthMethodView, AcpCommandView, AcpModeView,
    AcpModesView, AcpPermissionOptionView, AcpPermissionRequestView, AcpPermissionResolutionView,
    AcpPlanEntryView, AcpPromptImage, AcpPromptResult, AcpStartInfo, AcpStreamEvent,
    AcpToolCallView, AcpToolContentView, AcpToolLocationView, AcpUsageView, EventSink,
};
...
impl AcpSessionRunner {
    ...
    pub async fn prompt(
        ...
        .await?;
        rx.await.map_err(|_| "acp connection dropped".to_string())?
    }
    ...
    pub async fn new_session(&self, cwd: Option<PathBuf>) -> Result<AcpStartInfo, String> {
        let (reply, rx) = oneshot::channel();
        self.submit(SessionRequest::NewSession { cwd, reply })
            .await?;
        rx.await.map_err(|_| "acp connection dropped".to_string())?
    }
    ...
    pub fn respond_permission(
        ...
        let pending = self
            ...
            .remove(request_id)
            .ok_or_else(|| {
                format!("unknown or already resolved permission request `{request_id}`")
            })?;
...
async fn establish_session(
    ...
    let session: NewSessionResponse = connection
        .send_request(
            NewSessionRequest::new(context.cwd.clone()).mcp_servers(context.mcp_servers.clone()),
        )
        .block_task()
        .await?;
...
pub(crate) fn is_auth_required_error(error: &agent_client_protocol::Error) -> bool {
...
fn resolve_all_permissions_cancelled(
    ...
    let drained: Vec<(String, PendingPermission)> = pending.lock().unwrap().drain().collect();
...
pub(crate) fn auth_method_view(method: &AuthMethod) -> AcpAuthMethodView {
    AcpAuthMethodView {
