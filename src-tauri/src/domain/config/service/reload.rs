...
//!   `acp_agents.json`, `scripts.json`, and the agent context markdown are. `known_hosts.json` is deliberately excluded: it is
...
use crate::domain::config::service::io::read_json_or_default;
...
impl Storage {
    ...
    pub fn reload_config(&self, file: ConfigFile) -> ReloadOutcome {
        ...
        match file {
            ConfigFile::SshConfigs => match read_json_or_default::<Vec<crate::domain::ssh::model::SshConfig>>(&path)
            {
                Ok(next) => {
            ...
            ConfigFile::Scripts => {
                match read_json_or_default::<Vec<crate::domain::scripts::model::ScriptDefinition>>(&path) {
                    Ok(next) => {
            ...
            ConfigFile::AcpAgents => {
                match crate::domain::agent::model::load_agent_configs(&self.data_dir()) {
