...
mod tests {
    ...
    async fn connects_to_local_server_after_trusting_host_key() {
        ...
        let connection = crate::domain::ssh::service::transport::connect(&state, None, &config, CancellationToken::new())
            .await
            .expect("connect");
    ...
    async fn shutdown_closes_transport() {
        ...
        let connection = crate::domain::ssh::service::transport::connect(
            &state,
            None,
            &test_config(&server),
    ...
    async fn rejects_unknown_host_key_with_trust_payload() {
        ...
        let error = crate::domain::ssh::service::transport::connect(
            &state,
            None,
            &test_config(&server),
            CancellationToken::new(),
